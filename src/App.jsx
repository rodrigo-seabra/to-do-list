...
function App() {
  ...
  function AddTarefa() {
    if (tarefa.tarefa !== "") setListaTarefas([...Listatarefas, tarefa]); //Quando clicar no btn o setListaTarefas terá seu valor igual a tudo que vier antes do listatarefas mais a nova tarefa do input
    //Spread operator
  }
  ...
  function Excluirtarefa(id) {
    //Passa o id do elemento que foi clicado, ele que buscado pelo filter, quem satisfazer a condição fica na lista
        ...
        <div className={Style.addTask}>
          <div class={Style.inputwrapper}>
            <input
              ...
              } /*quando o valor do input mudar a cada clique, será chamado o setTarefa que o valor do setTarefa será o valor do input, e com isso é possível mudar o valor da const tarefa.*/
            />
          </div>
          <div className={Style.OrgButton}>
            <button onClick={AddTarefa} className={Style.comicButton}>
              Add
            </button>
          </div>
        </div>
      ...
      >
        <ul>
          ...
          {Listatarefas.map((item, index) => (
            <li
              className={item.status ? Style.risco : Style.semrisco}
              key={index}
            >
              {item.texto}
              <div className={Style.divbuttons} >
                <button onClick={() => statusTarefa(item.id, item.status)}>
                  {
                    /*Renderização condiconal:*/ item.status ? (
                      <img src={marcado} />
                    ) : (
                      <img src={desmarcado} />
                    ) /*Se status for true ele exibe 'Concluída, se for false 'Não concluída', além de exibir texto, é possível exibir icone ou mudar css de elementos de acordo com a condição, o ? é o if, e : é o else */
                  }
                </button>
                <button
                  type="submit"
                ...
                >
                  <img src={excluir} height="20px" width="20px" />
                </button>
              </div>
